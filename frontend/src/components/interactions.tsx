...
import { articles, newsTopics, steps } from '../lib/content';
...
// Menu links arrive as ?topic=<slug>. Only this reader waits on the URL, so the listing itself still prerenders.
function TopicFromUrl({ onTopic }: { onTopic: (label: string) => void }) {
  const topic = useSearchParams().get('topic');
  useEffect(() => { const match = newsTopics.find(t => t.slug === topic); if (match) onTopic(match.label); }, [topic, onTopic]);
  return null;
}
export function ArticleListing({ section }: { section: 'tech-blog' | 'news-insights' }) {
  const [query, setQuery] = useState(''); const [category, setCategory] = useState('All');
  const source = articles.filter(a => a.section === section); const categories = ['All', ...(section === 'news-insights' ? newsTopics.map(t => t.label) : new Set(source.map(a => a.category)))];
