...
export function TeamCarousel({ members }: { members: TeamMember[] }) {
  ...
  return <div className="team-carousel" role="region" aria-roledescription="carousel" aria-label="Our team">
    ...
    <div className="team-dots">{pages > 1 && Array.from({ length: pages }, (_, i) => <button key={i} type="button" aria-label={`Show position ${i + 1} of ${pages}`} aria-current={i === active || undefined} onClick={() => go(i)}/>)}</div>
