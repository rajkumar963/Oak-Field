...
export async function POST(request: NextRequest) {
  ...
  const name = typeof data.name === 'string' ? data.name.trim() : ''; const email = typeof data.email === 'string' ? data.email.trim() : ''; const message = typeof data.message === 'string' ? data.message.trim() : ''; const organization = typeof data.organization === 'string' ? data.organization.trim() : '';
  if(name.length < 2 || name.length > 120 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 254 || message.length < 20 || message.length > 5000 || organization.length > 120 || !['General','Media','Research collaboration','Careers'].includes(String(data.category)) || data.consent !== 'on') return reply('Please check the required fields and try again.',400);
