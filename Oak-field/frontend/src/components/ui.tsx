...
import { articles, type Article, footerColumns, legalLinks } from '../lib/content';
...
export function Footer() {
  return <footer className="site-footer">
    <div className="footer-art" aria-hidden="true"><span className="footer-cross"/><span className="footer-formula"><i>f</i>(<i>x</i>) = <span className="footer-sum"><small><i>n</i></small>∑<small><i>i</i>=1</small></span> <i>α<sub>i</sub></i> · <i>φ<sub>i</sub></i>(<i>x</i>)</span></div>
    <div className="wrap footer-main"><span className="footer-wave-wrap" aria-hidden="true"><FooterWave/></span>
      ...
      <nav className="footer-columns" aria-label="Footer">{footerColumns.map(col => <div key={col.title} className="footer-col"><h2>{col.title}</h2><ul>{col.links.map(l => <li key={l.label}><Link href={l.href} {...(l.external ? { target: '_blank', rel: 'noopener noreferrer' } : {})}>{l.icon && <span className="footer-icon">{footerIcons[l.icon]}</span>}<span>{l.label}</span><Arrow/></Link></li>)}</ul></div>)}</nav>
    </div>
    <div className="footer-bottom"><div className="wrap"><span className="footer-entity">OAK FIELD RESEARCH LLP</span><span className="footer-copy">© {new Date().getFullYear()} Oak Field Research LLP. All rights reserved.</span><nav aria-label="Legal">{legalLinks.map(l => <Link key={l.href} href={l.href}>{l.label}</Link>)}</nav></div></div>
  </footer>;
}
